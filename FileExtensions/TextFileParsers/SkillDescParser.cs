...
public static class SkillDescParser
{
    ...
    {
        var lines = (await TextFileParserFileUtility.ReadAllLinesAsync(typeof(SkillDescParser), path)).Skip(1); // Skip header
    ...
    public static Task<FileInfo> SaveEntries(IList<SkillDesc> entries, string? sourcePath = null, string? outputDirectory = null, CancellationToken cancellationToken = default)
    {
        return TextFileParserFileUtility.SaveEntriesAsync<SkillDesc>(typeof(SkillDescParser), entries, sourcePath, outputDirectory, cancellationToken);
    }
}
