...
/// <summary>
/// Parses and edits the D2R Reimagined <c>data/hd/character/monsters.json</c> file.
/// The file is a single JSON object whose top-level properties (excluding the
/// <c>dependencies</c> object) map a monster key to a string asset name.
...
/// </summary>
public class MonstersFileParser(string filePath)
{
    ...
    private async Task<JsonObject> LoadRootAsync()
    {
        ...
        if (rootNode is not JsonObject rootObject)
        {
            throw new InvalidDataException(
                $"{filePath} is not a JSON object and cannot be edited as a monsters file.");
        }
    ...
    /// </summary>
    public async Task<string?> GetMonsterValueByKeyAsync(string key)
    {
    ...
    /// <summary>
    /// Returns every monster key/value pair, skipping the reserved
    /// <c>dependencies</c> object and any non-string values.
    /// </summary>
    public async Task<IReadOnlyDictionary<string, string>> GetAllMonstersAsync()
    {
        var root = await LoadRootAsync();
    ...
    public async Task<bool> ReplaceMonsterValueAsync(string key, string? newValue)
    {
        if (string.IsNullOrWhiteSpace(key))
        {
            throw new ArgumentException("Key must be supplied", nameof(key));
    ...
    /// <summary>
    /// Adds a new monster entry to the end of the file. When the key already exists, the
    ...
    /// updated in place.
    /// </returns>
    public async Task<bool> AddMonsterAsync(string key, string? value)
    {
        if (string.IsNullOrWhiteSpace(key))
        {
            throw new ArgumentException("Key must be supplied", nameof(key));
